...
FORBIDDEN = (b"-----BEGIN PRIVATE KEY-----", b"-----BEGIN RSA PRIVATE KEY-----",
             b"-----BEGIN OPENSSH PRIVATE KEY-----", b"/Users/", b"/home/",
             b"../agentguard/", b"AGENTGUARD_CORE", b"github.com/TensorScholar/agentguard.git")
...
def collect(root: Path) -> dict[str, bytes]:
    ...
    for path in sorted(candidates):
        ...
        if relative != "scripts/build_release.py":
            ...
            if re.search(rb"(?:AKIA|ASIA)[A-Z0-9]{16}|gh[pousr]_[A-Za-z0-9]{30,}", data):
                raise ValueError(f"credential-like token detected: {relative}")
        files[relative] = data
    return files


def build(root: Path = ROOT) -> tuple[Path, str, int]:
    files = collect(root)
    ...
    archive = output / "agentguard-reference-final.zip"
    checksum_file = output / (archive.name + ".sha256")
    manifest_file = output / "manifest.sha256"
    for path in (archive, checksum_file, manifest_file):
        if path.is_symlink():
...
if __name__ == "__main__":
    archive, checksum, count = build()
    print(f"{archive.name}: {count} source files plus MANIFEST.sha256")
