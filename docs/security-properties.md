...
| Sequential and concurrent single-use dispatch | `tests/adversarial/test_scenarios.py`: `ScenarioTests.test_synthetic_local_replay_denied`; `tests/security/test_boundaries.py`: `BoundaryTests.test_replay_sixteen_threads_single_winner` |
...
| Caller-asserted provenance enforcement, not prompt classification | `tests/adversarial/test_scenarios.py`: `ScenarioTests.test_synthetic_untrusted_provenance_denied`, `ScenarioTests.test_synthetic_provenance_mutation_after_authorization_denied` |
